...
class Tecnologia(Base):
    ICONE_CHOICES = (
        ...
        ('lni-leaf', _('Design Moderno')),
        ('lni-layers', _('Modelo Multiuso')),
        ('lni-user', _('Formulario')),
        ('lni-rocket', _('Foguete')),
    )
    tecnologia = models.CharField(_('Tecnologia'), max_length=100)
    descricao = models.TextField(_('Descrição'), max_length=200)
    icone = models.CharField(_('Icone'), max_length=16, choices=ICONE_CHOICES)

    class Meta:
        verbose_name = _('Tecnologia')
        verbose_name_plural = _('Tecnologias')
...
class Funcionario(Base):
    ...
    cargo = models.ForeignKey(
        'core.Cargo', verbose_name=_('Cargo'), on_delete=models.CASCADE)
    bio = models.TextField(_('Bio'), max_length=200)
    imagem = StdImageField(_('Imagem'), upload_to=get_file_path, variations={
